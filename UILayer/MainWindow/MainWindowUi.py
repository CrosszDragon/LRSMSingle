...
class MainWindowUI(object):
    ...
    # 创建快速选择的工具栏
    def _init_select_toolbar(self, main_window: QMainWindow):
        # 添加 快速选择 的工具栏
        ...
        self.add_to_selection_action = create_action(
            ...
            image=os.path.join(BASE_DIR, "sources/icons/new_selection.ico"))
        self.sub_from_selection_action = create_action(
            main_window, "", tip="从选区移除", checkable=True,
            image=os.path.join(BASE_DIR, "sources/icons/new_selection.ico"))
        self.and_with_selection_action = create_action(
            main_window, "", tip="与选区交叉",  checkable=True,
            image=os.path.join(BASE_DIR, "sources/icons/new_selection.ico"))
        selection_actions = (self.new_selection_action,
                             self.add_to_selection_action,
                             self.sub_from_selection_action,
                             self.and_with_selection_action)
        # 加入组
    ...
    # 创建快速选择的工具栏
    def _init_zoom_toolbar(self, main_window: QMainWindow):
        # 添加 快速选择 的工具栏
        self.zoom_toolbar = main_window.addToolBar("zoom")
        toolbar_stylesheet = """
        QCheckBox { margin: 0 10px; }
        QLabel { margin-left: 8px; }
        QLineEdit { margin-right: 7px; }
        """
        self.zoom_toolbar.setStyleSheet(toolbar_stylesheet)

        # 点击放大缩小动作
        self.zoom_in_action = create_action(
            main_window, "", tip="放大",  checkable=True,
            image=os.path.join(BASE_DIR, "sources/icons/zoom_in.svg"))
        self.zoom_out_action = create_action(
            main_window, "", tip="缩小", checkable=True,
            image=os.path.join(BASE_DIR, "sources/icons/zoom_out.svg"))

        join_group(QActionGroup(self.zoom_toolbar), (self.zoom_in_action, self.zoom_out_action))
        add_actions(self.zoom_toolbar, (self.zoom_in_action, self.zoom_out_action))
        self.zoom_in_action.setChecked(True)
        self.zoom_toolbar.addSeparator()

        self.resize_window_action = QCheckBox()
        self.resize_window_action.setText("调整窗口大小以全屏显示")
        self.zoom_toolbar.addWidget(self.resize_window_action)
        self.scale_all_window_action = QCheckBox()
        self.scale_all_window_action.setText("缩放所有窗口")

        self.zoom_toolbar.addWidget(self.scale_all_window_action)
        self.scrubby_zoom_action = QCheckBox()
        self.scrubby_zoom_action.setText("Scrubby Zoom")
        self.zoom_toolbar.addWidget(self.scrubby_zoom_action)
        self.zoom_toolbar.addSeparator()

        self.actual_pixels_action = create_action(main_window, "实际像素", tip="将当前窗口缩放为1:1")
        self.suitable_for_screen_action = create_action(main_window, "适合屏幕", tip="将当前窗口缩放为屏幕大小" )
        self.fill_screen_action = create_action(main_window, "填充屏幕", tip="缩放当前窗口以适合屏幕" )
        self.print_size_action = create_action(main_window, "打印尺寸", tip="将当前窗口缩放为打印尺寸")

        add_actions(self.zoom_toolbar, (self.actual_pixels_action, self.suitable_for_screen_action,
                                        self.fill_screen_action, self.print_size_action))
