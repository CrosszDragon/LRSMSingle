...
from UILayer.MainWindow.MainToolBar import ToolsToolBar, SelectionOptionToolBar

from CommonHelper.CommonHelper import qimage2numpy
from CommonHelper.MatToImg import mat_to_img
...
# from Algorithm.AIAlgorithm.AiDetect import detect_image, detect_one
...
class Document(QWidget, ProjectDocument):
    ...
    def __init__(self, gadget, toolbar_gadget, file_name=None, project_name=None,
        ...
        if self._is_big_img:
            self.workbench_view = LoadIMGraphicsView(gadget, toolbar_gadget, eraser_size,
                                                     image_path, self._workbench_scene, parent=self)
        else:
            self.workbench_view = GraphicsViewTest(gadget, toolbar_gadget, eraser_size, parent=self)
            # 把场景添加到视图中
    ...
    def ai_delete_outline(self, detect_policy):
        ...
        if detect_policy == 5:
            for h in range(0, self._image.height(), 256):
                for w in range(0, self._image.width(), 256):
                    image = self._image.copy(QRect(w, h, 255, 255))
                    image = qimage2numpy(image)
                    # result = detect_one(image)
                    ...
                    print("width_num_array shape: ", width_num_array.shape)
                ...
                print("result_numpy_array shape: ", result_numpy_array.shape)
                width_num_array = None
